# main.py
# BibleAI v281.36.목사님의 교회 - 사전 검색 기능 수정 버전
...
# --- [1. 시스템 설정 및 세션 초기화] ---
st.set_page_config(page_title="목사님의 교회", layout="wide")
    ...
    # [D] 일반 검색 실행 (개선된 주제어 검색 로직)
        ...
        with st.chat_message("ai"):
            ...
            file_results = search_files_advanced(
                user_input,
                selected_folders if selected_folders else ["."]
            )

            total_results = bible_results + file_results
            ...
            if not total_results:
                ...
                for i, item in enumerate(total_results):
                    with st.container(border=True):
                        ...
                        # [NEW 4, 5] 복사 및 AI 요약 버튼 추가
                        col1, col2, col3 = st.columns(3)

                        with col1:
                            # [NEW 4] 복사 버튼
                            if st.button(f"📋 복사", key=f"copy_btn_{i}"):
                                try:
                        ...
                        with col2:
                            # [NEW 5] AI 요약 버튼
                            if st.button(f"🤖 요약", key=f"ai_sum_{i}"):
                                with st.spinner("요약 중..."):
                                    summary = summarize_with_ai(item['content'], st.session_state.selected_model)
                                    st.info(f"💡 {summary}")

                        with col3:
                            # [🧺 바구니에 담기 버튼]
                            if st.button(f"🧺 담기", key=f"btn_add_{i}"):
                                if item not in st.session_state.basket:
                                    st.session_state.basket.append(item)
                                    # [NEW 3] 현재 그룹에도 추가
                                    st.session_state.basket_groups[st.session_state.current_group].append(item)
                                    st.success(f"'{item['file']}'을(를) '{st.session_state.current_group}'에 담았습니다!")
                                else:
                                    st.warning("이미 담겨 있는 자료입니다.")
